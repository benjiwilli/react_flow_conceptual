...
import type { WorkflowNode, NodeData } from "@/lib/types"
...
export default function NodeConfigPanel({ node, updateNodeData, onClose }: NodeConfigPanelProps) {
  const [localData, setLocalData] = useState<Partial<NodeData>>({ ...node.data })

  const handleChange = (key: keyof NodeData, value: NodeData[keyof NodeData]) => {
    setLocalData((prev) => ({
      ...prev,
      [key]: value,
    }))
    updateNodeData(node.id, { [key]: value } as Partial<NodeData>)
  }
