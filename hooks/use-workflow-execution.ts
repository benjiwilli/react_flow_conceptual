...
export function useWorkflowExecution(options: UseWorkflowExecutionOptions = {}): UseWorkflowExecutionReturn {
  ...
  const execute = useCallback(
    async (workflow: LinguaFlowWorkflow, studentId: string) => {
      ...
      try {
        // Execute workflow via API (streaming via SSE in session page)
  ...
  const pause = useCallback(() => {
    setIsPaused(true)
    // Note: Server-side pause handled via SSE abort; full pause/resume requires WebSocket (future enhancement)
  }, [])

  const resume = useCallback(async () => {
    setIsPaused(false)
    // Note: Resume re-initiates workflow from checkpoint; full state preservation requires WebSocket (future enhancement)
  }, [])
